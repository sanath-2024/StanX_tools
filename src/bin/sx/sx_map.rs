...
pub fn map(
    ...
    if phase <= 2 {
        println!("\n\nPHASE 2\n");
        transposons_map = Some(select_reads::select_reads(
            &te_aligned_path,
            &selected_reads_path,
            false,
        ));
    ...
    if phase <= 4 {
        ...
        select_alignments::select_alignments(
            ...
            &genome_aligned_path,
            &output_path,
            &transposons_map.unwrap_or_else(|| {
                select_reads::select_reads(&te_aligned_path, &selected_reads_path, true)
            }),
